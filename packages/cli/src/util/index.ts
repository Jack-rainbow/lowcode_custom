...
export function printPkgVersion () {
  const PaasVersion = getPkgVersion()
  console.log(`🐫 Paas v${PaasVersion}`)
}
...
export function getpnpmLock (): boolean {
  const pkgPath = path.join(path.join(getRootPath(), 'pnpm-lock.yaml'))
